<template>
  <div class="v-cal">
    <header class="v-cal-header">
      <div class="v-cal-header__actions" v-if="showActions">
...
export default {
  ...
  methods: {
    ...
    bindEvents() {
      ...
      EventBus.$on('calendar-go-to-today', (event) => {
        this.goToToday();
      });
      EventBus.$on('calendar-next', (event) => {
        this.next();
      });
      EventBus.$on('calendar-prev', (event) => {
        this.prev();
      });
      EventBus.$on('calendar-switch-view', (data) => {
  ...
  computed: {
    ...
    isPrevAllowed() {
      if ( this.minDate ) {
        ...
        EventBus.$emit('calendar-is-prev-allowed', this.minDate.isSameOrBefore(prevRef, this.activeView));
        return this.minDate.isSameOrBefore(prevRef, this.activeView);
      }
      EventBus.$emit('calendar-is-prev-allowed', true);
      return true
    },
    isNextAllowed() {
      if ( this.maxDate ) {
        const afterRef = moment(this.activeDate).add(1, this.activeView + 's');
        EventBus.$emit('calendar-is-next-allowed', this.maxDate.isSameOrAfter(afterRef, this.activeView));
        return this.maxDate.isSameOrAfter(afterRef, this.activeView);
      }
      EventBus.$emit('calendar-is-next-allowed', true);
      return true
    },
    activeViewProps() {
      let props = {
